...
{
    const string SaveFolder = "GameProject/Script/ActGame/ConfigData";
    const string Space4 = "    ";
    const string Space8 = "        ";
    ...
    private static string GetClassContent(List<string[]> content, string className)
    {
        ...
        sb.AppendLine(string.Format("public partial class {0}:ConfigDataBase", className));
        sb.AppendLine("{");
        ...
        for (int i = 0; i < fieldNames.Length; i++)
        {
            ...
            if (string.IsNullOrEmpty(fieldName))
                continue;
            sb.AppendLine(string.Format("{0}/// <summary>", Space4));
            sb.AppendLine(string.Format("{1}///{0}", fieldDescs[i], Space4));
            sb.AppendLine(string.Format("{0}/// </summary>", Space4));
            sb.AppendLine(string.Format("{0}[Field]", Space4));
